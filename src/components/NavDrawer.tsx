...
import { MenuIcon, PhoneIcon, MailIcon, XIcon, MapPinIcon, ChevronDownIcon } from "lucide-react"
import {
	NavigationMenu,
	NavigationMenuItem,
	NavigationMenuLink,
							...
							<NavigationMenu orientation="vertical" className="items-start">
								<NavigationMenuList className="flex w-full flex-col items-center gap-4">
									{linkItems.map(({ href, title, isCta, children }) =>
										children ? (
											<Collapsible key={title} className="flex flex-col items-center gap-y-3">
											...
											<NavigationMenuItem key={title}>
												<NavigationMenuLink asChild>
													<Anchor href={href} variant={"default"} className="text-xl font-medium">
														{title}
													</Anchor>
												</NavigationMenuLink>
											</NavigationMenuItem>
			...
			<li>
				<NavigationMenuLink asChild>
					...
					>
						...
						<p className="line-clamp-2 text-center leading-snug text-muted-foreground">{children}</p>
					</a>
				</NavigationMenuLink>
			</li>
		)
