				...
				{linkItems.map(({ href, title, isCta, children }) =>
					children ? (
						<NavigationMenuItem key={title}>
							<NavigationMenuTrigger className="bg-transparent p-0 text-lg font-medium">{title}</NavigationMenuTrigger>
							<NavigationMenuContent>
						...
						<NavigationMenuItem key={title}>
							<NavigationMenuLink asChild>
								<Anchor href={href} variant={"default"} size={"lg"} className="text-lg font-bold">
									{title}
								</Anchor>
							</NavigationMenuLink>
						</NavigationMenuItem>
